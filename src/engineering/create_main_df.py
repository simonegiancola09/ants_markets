...
def load_R_number_data():
    ...
    df_covid_US.index.names = ['Date'] # set name of index to Date in caps                      
...
def retrieve_cases_and_R_number():
    ...
    '''
    # locations of previously downloaded files
    path_covid_cases = 'data/raw/'
    path_R_number = 'data/engineered/'
    filename_covid_cases = 'covid_US_raw.csv'
    filename_R_number = 'R_number_data.csv'
    # read csvs
    df_covid_US = pd.read_csv(path_covid_cases + filename_covid_cases,
                              index_col='date')
    df_R_number = pd.read_csv(path_R_number + filename_R_number, 
                              index_col=0 # date is first column in df
                              )
    # naming convention again
    df_R_number.index.names = ['Date']
